# -*- coding: utf-8 -*-
"""avendano1daniel (Apr 24, 2025, 2:47:54 PM)
...
def load_data_local(file_path):
    # Only read the main 3 features and limit to first 14,000 rows
...
# Load real and synthetic benign samples
real_df = load_data_local(files['benign'])  # <--- Make sure to have set up the path to this file
synthetic_df = load_data_local(files['synthetic']) # <--- Make sure to have set up the path to this file

# Split synthetic into training and validation (80/20)
s_train, s_val = train_test_split(synthetic_df, test_size=0.2, random_state=42)

# Use all real data as the independent test set
test_df = real_df.reset_index(drop=True)

# Shuffle train and val sets
train_df = s_train.sample(frac=1, random_state=42).reset_index(drop=True)
val_df   = s_val.sample(frac=1, random_state=42).reset_index(drop=True)
...
thresholds = {m: np.mean(v) + 2.5 * np.std(v) for m, v in test_errors.items()}  # <--- Threshold can be changed
...
print("\nImageNet Classification Report")
